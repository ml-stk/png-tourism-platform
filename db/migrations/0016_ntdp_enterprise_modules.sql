-- NTDP Concept Note compliance baseline: enterprise modules not yet represented
-- as first-class persistence domains.
...
create table if not exists analytics.daily_metric_fact (
  ...
  primary key (metric_date, metric_code, province_code)
);
...
create table if not exists sme_development_programs (
  ...
  start_date date,
  end_date date,
  status text not null check (status in ('planned','open','closed','completed')) default 'planned'
);
...
create table if not exists tia_memberships (
  ...
  status text not null check (status in ('applied','active','expired','suspended','cancelled')) default 'applied',
  applied_at timestamptz not null default now(),
  approved_at timestamptz,
  expires_at date,
  fee_amount numeric(12,2),
  fee_currency char(3) default 'PGK',
  renewal_of uuid references tia_memberships(id),
  notes text,
  created_at timestamptz not null default now(),
  updated_at timestamptz not null default now()
);
...
create table if not exists distribution.publications (
  ...
  status text not null check (status in ('queued','published','failed','withdrawn')) default 'queued',
...
create table if not exists commerce.transactions (
  id uuid primary key default gen_random_uuid(),
  provider_id uuid references commerce.payment_providers(id),
  operator_id uuid references operators(id),
  external_reference text,
  transaction_type text not null check (transaction_type in ('membership_fee','application_fee','booking','other')),
...
insert into distribution.channels(code,name,channel_type,status,requires_authentication) values
 ('png-tourism-website','PNG Tourism Website','website','active',false),
 ('png-tourism-super-app','PNG Tourism Super App','super_app','draft',true),
 ('provincial-portals','Provincial Tourism Portals','provincial_portal','active',true),
 ('tourism-kiosks','Tourism Information Kiosks','kiosk','active',true),
 ('approved-third-party','Approved Third-Party Platforms','third_party','draft',true)
on conflict (code) do nothing;
insert into commerce.payment_providers(code,name,status,capabilities)
values ('platform-ready','NTDP Commerce Readiness','planned','["membership_fee","application_fee","booking","refund"]'::jsonb)
on conflict (code) do nothing;

insert into permissions (code, description) values
 ('gis:read','Read governed tourism geospatial assets'),('gis:write','Create and update tourism geospatial assets'),
 ('sme:read','Read SME development records'),('sme:write','Create and update SME development records'),
 ('membership:read','Read TIA membership records'),('membership:write','Create and manage TIA membership applications'),
 ('distribution:read','Read distribution channel and publication records'),('distribution:write','Queue approved tourism content for distribution'),
 ('commerce:read','Read commerce readiness and transaction status'),('commerce:write','Create governed commerce transactions'),
 ('warehouse:read','Read analytical warehouse metrics'),('warehouse:write','Run governed analytical snapshots')
on conflict (code) do nothing;
insert into role_permissions (role_id, permission_id)
select r.id,p.id from roles r join permissions p on p.code in ('gis:read','sme:read','membership:read','distribution:read','commerce:read','warehouse:read') where r.code in ('platform_admin','analyst') on conflict do nothing;
insert into role_permissions (role_id, permission_id)
select r.id,p.id from roles r join permissions p on p.code in ('gis:read','gis:write','sme:read','sme:write','distribution:read','distribution:write') where r.code='provincial_admin' on conflict do nothing;
insert into role_permissions (role_id, permission_id)
select r.id,p.id from roles r join permissions p on p.code in ('membership:read','membership:write','commerce:read','commerce:write','warehouse:read','warehouse:write','gis:read','gis:write','distribution:read','distribution:write') where r.code='tpa_regulator' on conflict do nothing;
insert into role_permissions (role_id, permission_id)
select r.id,p.id from roles r join permissions p on p.code in ('sme:read','sme:write','membership:read','distribution:read','commerce:read','gis:read') where r.code='operator' on conflict do nothing;
