...
begin
  update regulatory_compliance_actions set status='overdue', updated_at=now()
...
declare affected integer;
begin
  update regulatory_licenses set status='expired', updated_at=now()
...
begin
  ...
  set compliance_status = case
    when exists (select 1 from regulatory_compliance_actions a where a.operator_id=o.id and a.status in ('open','in_progress','overdue')) then 'non_compliant'::compliance_status
    when exists (select 1 from regulatory_licenses l where l.operator_id=o.id and l.status='approved' and (l.expires_at is null or l.expires_at >= now())) then 'compliant'::compliance_status
    when exists (select 1 from regulatory_licenses l where l.operator_id=o.id and l.status='approved' and l.expires_at is not null and l.expires_at < now()) then 'non_compliant'::compliance_status
