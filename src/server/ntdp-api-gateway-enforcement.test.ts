...
vi.mock('../services/ntdp-api-gateway-service', () => ({
  NtdpApiGatewayService: class {
    routes(...args: unknown[]) { return service.routes(...args); }
    clients(...args: unknown[]) { return service.clients(...args); }
    createClient(...args: unknown[]) { return service.createClient(...args); }
    transitionClient(...args: unknown[]) { return service.transitionClient(...args); }
    keys(...args: unknown[]) { return service.keys(...args); }
    issueKey(...args: unknown[]) { return service.issueKey(...args); }
    revokeKey(...args: unknown[]) { return service.revokeKey(...args); }
    usage(...args: unknown[]) { return service.usage(...args); }
    authenticateApiKey(...args: unknown[]) { return service.authenticateApiKey(...args); }
    logRequest(...args: unknown[]) { return service.logRequest(...args); }
  },
}));
...
function request(method: string, path: string, headers: Record<string, string> = {}, body = ''): IncomingMessage {
  const req = { method, url: path, headers, [Symbol.asyncIterator]: async function* () { if (body) yield Buffer.from(body); } } as unknown as IncomingMessage;
  return req;
}
function response() {
  return { setHeader: vi.fn(), end(this: MockResponse, value?: string) { this.body = value; }, statusCode: 0 } as unknown as MockResponse;
}
...
describe('NTDP API gateway administration', () => {
  it('requires authentication for gateway administration', async () => {
    ...
    const res = response();
    await handleNtdpApiGatewayApi(request('GET', '/api/v1/ntdp/gateway/routes'), res);
  ...
  it('rejects malformed client creation', async () => {
    ...
    const res = response();
    await handleNtdpApiGatewayApi(request('POST', '/api/v1/ntdp/gateway/clients', {}, '{}'), res);
  ...
  it('requires gateway write permission for key issuance', async () => {
    ...
    const res = response();
    await handleNtdpApiGatewayApi(request('POST', '/api/v1/ntdp/gateway/keys', {}, JSON.stringify({ clientId: 'c1' })), res);
  ...
  it('never exposes an issued key from a subsequent list operation', async () => {
    ...
    const res = response();
    await handleNtdpApiGatewayApi(request('GET', '/api/v1/ntdp/gateway/keys'), res);
