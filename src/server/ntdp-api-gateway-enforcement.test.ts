...
import { handleNtdpApiGatewayApiWithService } from './ntdp-api-gateway-api';
...
describe('NTDP API gateway administration boundary', () => {
  ...
  it('requires authentication for gateway administration', async () => {
    ...
    const res = response();
    await handleNtdpApiGatewayApiWithService(request('GET','/api/v1/ntdp/gateway/routes'),res,service as never);
    expect(res.statusCode).toBe(401);
  });

  it('rejects malformed client creation', async () => {
    const res = response();
    await handleNtdpApiGatewayApiWithService(request('POST','/api/v1/ntdp/gateway/clients',{},'{}'),res,service as never);
  ...
  it('requires gateway write permission for key issuance', async () => {
    ...
    const res = response();
    await handleNtdpApiGatewayApiWithService(request('POST','/api/v1/ntdp/gateway/keys',{},JSON.stringify({clientId:'c1'})),res,service as never);
  ...
  it('never exposes an issued key from a subsequent list operation', async () => {
    ...
    const res = response();
    await handleNtdpApiGatewayApiWithService(request('GET','/api/v1/ntdp/gateway/keys'),res,service as never);
