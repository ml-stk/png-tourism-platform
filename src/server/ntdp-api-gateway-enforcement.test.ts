...
function response(){ const res = { setHeader: vi.fn(), end(this: MockResponse, value?: string){ this.body=value; }, statusCode: 0 } as unknown as MockResponse; return res; }

describe('NTDP API gateway enforcement', () => {
  it('requires authentication for gateway administration', async () => {
    const api = await import('./api'); vi.mocked(api.authenticate).mockRejectedValueOnce(Object.assign(new Error('no auth'), { code: 'UNAUTHORIZED' }));
