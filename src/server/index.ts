...
async function dispatch(req:import('node:http').IncomingMessage,res:import('node:http').ServerResponse,existingHandler:(req:import('node:http').IncomingMessage,res:import('node:http').ServerResponse)=>void,routeKey:string|undefined,clientId:string|undefined,apiKeyId:string|undefined,originalUrl:string,gateway:NtdpApiGatewayService,started:number,requestId:string){
  ...
  await withNtdpPagination(req,res,()=>routeHandlers(req,res));
...
async function routeHandlers(req:import('node:http').IncomingMessage,res:import('node:http').ServerResponse){if(await handleNtdpApiGatewayApi(req,res))return;if(await handleNtdpDistributionApi(req,res))return;if(await handleNtdpCommerceApi(req,res))return;if(await handleNtdpRegulatoryApi(req,res))return;if(await handleNtdpEnterpriseApi(req,res))return;if(await handleOperatorRegistrationApi(req,res))return;if(await handleIndustryApi(req,res))return;if(await handleVisitorEngagementApi(req,res))return;if(await handlePassportApi(req,res))return;if(await handleCommandCentreApi(req,res))return;if(await handleCampaignEventApi(req,res))return;if(await handleContentStudioApi(req,res))return;if(await handleDestinationPublicApi(req,res))return;if(await handleMediaApi(req,res))return;res.statusCode=404;res.end(JSON.stringify({error:{code:'NOT_FOUND',message:'Route not found'}}));}
