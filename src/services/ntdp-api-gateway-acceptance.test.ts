...
describe('NtdpApiGatewayService authorization acceptance', () => {
  it('allows an unkeyed request to an active public route', async () => {
    const { service } = service([{ route_key: 'public-destination-list', visibility: 'public', required_scope: null, status: 'active', path_pattern: '/api/v1/public/destinations' }]);
  ...
  it('rejects an unkeyed request to a protected route', async () => {
    const { service } = service([{ route_key: 'gateway-usage', visibility: 'protected', required_scope: 'gateway:read', status: 'active', path_pattern: '/api/v1/ntdp/gateway/usage' }]);
  ...
  it('matches parameterized routes without accepting extra path segments', async () => {
    const { service } = service([{ route_key: 'public-destination-detail', visibility: 'public', required_scope: null, status: 'active', path_pattern: '/api/v1/public/destinations/{id}' }]);
  ...
  it('does not allow a disabled route', async () => {
    const { service } = service([{ route_key: 'disabled', visibility: 'public', required_scope: null, status: 'disabled', path_pattern: '/api/v1/public/disabled' }]);
