...
export class OperatorService {
  ...
  async register(input: { legalName: string; tradingName?: string; provinceCode: ProvinceCode; actorId?: string; requestId?: string }): Promise<Operator> {
    const legalName = input.legalName.trim();
    if (!legalName) throw new DomainError('VALIDATION_ERROR', 'Legal name is required');
    const now = new Date().toISOString();
    const operator: Operator = { id: crypto.randomUUID(), legalName, tradingName: input.tradingName?.trim() || undefined, provinceCode: input.provinceCode, status: 'pending_review', complianceStatus: 'unknown', createdAt: now, updatedAt: now };
  ...
  async reject(id: string, reason: string, actorId?: string, requestId?: string): Promise<Operator> {
    return this.transitionWithReason(id, 'pending_review', 'closed', 'operator.rejected', reason, actorId, requestId);
  }
  ...
  async updateCompliance(id: string, complianceStatus: ComplianceStatus, note: string | undefined, actorId?: string, requestId?: string): Promise<Operator> {
    if (!['unknown', 'compliant', 'conditional', 'non_compliant'].includes(complianceStatus)) throw new DomainError('VALIDATION_ERROR', 'Invalid compliance status');
    ...
    await this.audit.record({ actorId, action: 'operator.compliance_updated', targetType: 'operator', targetId: id, outcome: 'success', requestId, metadata: note ? { note: note.trim() } : undefined });
    return updated;
  }

  async suspend(id: string, reason: string, actorId?: string, requestId?: string): Promise<Operator> {
    return this.transitionWithReason(id, 'active', 'suspended', 'operator.suspended', reason, actorId, requestId);
  }

  async close(id: string, reason: string, actorId?: string, requestId?: string): Promise<Operator> {
    return this.transitionWithReason(id, 'suspended', 'closed', 'operator.closed', reason, actorId, requestId);
  }

  private async transition(id: string, expectedStatus: OperatorStatus, nextStatus: OperatorStatus, action: string, actorId?: string, requestId?: string): Promise<Operator> {
    const operator = await this.get(id);
    if (operator.status !== expectedStatus) throw new DomainError('CONFLICT', `Operator must be ${expectedStatus} to become ${nextStatus}`);
    return this.applyTransition(operator, expectedStatus, nextStatus, action, actorId, requestId);
  }

  private async transitionWithReason(id: string, expectedStatus: OperatorStatus, nextStatus: OperatorStatus, action: string, reason: string, actorId?: string, requestId?: string): Promise<Operator> {
    const normalizedReason = reason.trim();
    if (!normalizedReason) throw new DomainError('VALIDATION_ERROR', `${nextStatus === 'closed' ? 'Closure' : 'Suspension'} reason is required`);
    const operator = await this.get(id);
    if (operator.status !== expectedStatus) throw new DomainError('CONFLICT', `Operator must be ${expectedStatus} to become ${nextStatus}`);
    return this.applyTransition(operator, expectedStatus, nextStatus, action, actorId, requestId, { reason: normalizedReason });
  }

  private async applyTransition(operator: Operator, expectedStatus: OperatorStatus, nextStatus: OperatorStatus, action: string, actorId?: string, requestId?: string, metadata?: Record<string, unknown>): Promise<Operator> {
    ...
    await this.audit.record({ actorId, action, targetType: 'operator', targetId: operator.id, outcome: 'success', requestId, metadata });
    return updated;
  }
}
