...
describe('OfflineService public delivery',()=>{
 ...
 it('creates a versioned verified deep-link handoff',()=>{const service=new OfflineService(deps());const handoff=service.createQrHandoff('content','content-1');expect(handoff.version).toBe(1);expect(handoff.targetType).toBe('content');expect(handoff.targetId).toBe('content-1');expect(handoff.verificationToken).toEqual(expect.any(String));expect(handoff.expiresAt).toEqual(expect.any(String));expect(handoff.uri).toContain('pngtourism://handoff/content/content-1?verification=');});
});
