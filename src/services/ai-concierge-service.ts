...
export interface AiConciergeDeps { destinations: DestinationRepository; content: ContentRepository; operators: OperatorRepository; generate?: (input: { message: string; toolResults: AiToolResult[]; promptVersion: string }) => Promise<AiAnswer>; }
...
export class AiConciergeService {
  ...
  async executeTool(request: AiToolRequest): Promise<AiToolResult> {
    ...
    const provinceCode = typeof request.input.provinceCode === 'string' ? request.input.provinceCode as ProvinceCode : undefined;
    if (!query) return { toolName: request.toolName, output: [], sources: [] };
    if (request.toolName === 'search_destinations') {
      const result = await this.deps.destinations.list({ provinceCode, publicationStatus: 'published', limit: 1000 });
      const items = result.items.filter((item) => item.publicationStatus === 'published' && item.name.toLowerCase().includes(query));
      const sources = items.map((item) => ({ id: item.id, kind: 'public_destination' as const, title: item.name, provenance: `destination:${item.id}:v${item.contentVersion}`, publicationStatus: 'published' as const }));
      return { toolName: request.toolName, output: items, sources };
    }
    if (request.toolName === 'search_experiences') {
      const result = await this.deps.content.list({ publicationStatus: 'published', provinceCode, limit: 1000 });
    ...
    const result = await this.deps.operators.list({ provinceCode, status: 'active', limit: 1000 });
    const items = result.items.filter((item) => item.status === 'active' && (item.tradingName ?? item.legalName).toLowerCase().includes(query));
    ...
    return { toolName: request.toolName, output: items.map(({ id, legalName, tradingName, provinceCode: itemProvince }) => ({ id, legalName, tradingName, provinceCode: itemProvince })), sources };
  }
  async answer(request: AiConversationRequest): Promise<AiConversationResponse> {
    ...
    const scope = request.provinceCode ? { provinceCode: request.provinceCode } : {};
    if (/destination|where|visit|place|attraction/.test(lower)) toolRequests.push({ toolName: 'search_destinations', input: { query: message, ...scope } });
    if (/experience|activity|do|adventure/.test(lower)) toolRequests.push({ toolName: 'search_experiences', input: { query: message, ...scope } });
    if (/operator|tour company|tour operator/.test(lower)) toolRequests.push({ toolName: 'find_operator', input: { query: message, ...scope } });
    ...
    if (this.deps.generate) { const generated = await this.deps.generate({ message, toolResults, promptVersion: PROMPT_VERSION }); return { ...generated, sessionId, modelVersion: generated.modelVersion || MODEL_VERSION, promptVersion: PROMPT_VERSION, governed: true, sources: generated.sources.length ? generated.sources : sources }; }
