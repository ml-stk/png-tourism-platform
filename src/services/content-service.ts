...
import type { AuditWriter, DestinationRepository } from './contracts';
...
export class DestinationService {
  ...
  async create(input: { name: string; slug: string; provinceCode: ProvinceCode; description?: string; latitude?: number; longitude?: number; actorId: string; requestId?: string }) {
    ...
    const item: Destination = { id: crypto.randomUUID(), name, slug, provinceCode: input.provinceCode, publicationStatus: 'draft', description: input.description?.trim() || undefined, latitude: input.latitude, longitude: input.longitude };
  ...
  async update(id: string, patch: Partial<Pick<Destination, 'name'|'slug'|'description'|'latitude'|'longitude'|'provinceCode'>>, actorId: string, requestId?: string) {
    ...
    const updated = await this.repository.update({ ...current, ...patch, name: patch.name?.trim() || current.name, slug: patch.slug?.trim().toLowerCase() || current.slug });
  ...
  async setPublication(id: string, status: PublicationStatus, actorId: string, requestId?: string) {
    ...
    const updated = await this.repository.update({ ...current, publicationStatus: status });
    if (!updated) throw new DomainError('CONFLICT', 'Destination changed; retry with current state');
...
export class ContentService {
  ...
  async create(input: { type: ContentItem['type']; title: string; slug: string; actorId: string; requestId?: string }) {
    ...
    const item: ContentItem = { id: crypto.randomUUID(), type: input.type, title, slug, publicationStatus: 'draft', version: 1, updatedAt: new Date().toISOString() };
  ...
  async setPublication(id: string, status: PublicationStatus, actorId: string, requestId?: string) {
    ...
    const updated = await this.repository.update({ ...current, publicationStatus: status, version: current.version + 1, updatedAt: new Date().toISOString() }, current.version);
    if (!updated) throw new DomainError('CONFLICT', 'Content changed; retry with current state');
    await this.audit.record({ actorId, action: `content.${status}`, targetType: 'content', targetId: id, outcome: 'success', requestId: requestId }); return updated;
  }
}
