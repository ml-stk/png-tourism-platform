...
export interface PublicVisitorServiceDeps {
...
export class PublicVisitorService {
  ...
  async destinationsList(options?: { provinceCode?: string; cursor?: string; limit?: number }) {
    return this.deps.destinations.list({ ...options, publicationStatus: 'published' } as Parameters<DestinationRepository['list']>[0]);
  }
  ...
  async operatorsList(options?: { provinceCode?: string; cursor?: string; limit?: number }) {
    ...
    return { ...result, items: result.items.filter(isPublicOperator) };
  }

  async operator(id: string): Promise<Pick<Operator, 'id' | 'tradingName' | 'provinceCode'>> {
    ...
    return { id: item.id, tradingName: item.tradingName, provinceCode: item.provinceCode };
  }
}

function isPublicOperator(operator: Operator) {
  return operator.status === 'active' && operator.complianceStatus === 'compliant' && Boolean(operator.tradingName);
}
