...
function normalizePlaces(value: unknown): Place[] {
  ...
  return value.map((item: any) => ({
    id: String(item.id), name: String(item.name), province: String(item.provinceCode ?? item.province ?? ''),
...
export default function TripPlanner() {
  ...
  const loadDestinations = async () => {
    setLoading(true);
    try {
      const response = await fetchWithTimeout('/api/v1/public/destinations', { headers: { Accept: 'application/json' } });
      ...
      setError('Live destinations are taking too long to respond. Showing the governed visitor-safe destination set.');
  ...
  const mapBounds = useMemo(() => {
    ...
    const latitudes = source.map(p => p.latitude), longitudes = source.map(p => p.longitude);
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
      ...
      <section className="rounded-2xl border border-slate-200 bg-white p-5 lg:p-6"><div className="flex items-center justify-between"><div><h3 className="text-xl font-bold">Your journey</h3><p className="mt-1 text-sm text-slate-500">{stops.length} stop{stops.length === 1 ? '' : 's'} · reorderable · offline-capable</p></div>{stops.length > 0 && <button onClick={reset} className="inline-flex items-center gap-1 text-xs font-semibold text-slate-500"><RotateCcw size={13}/> Reset</button>}</div><div className="mt-5 space-y-3">{stops.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">Add a destination to start your itinerary.</div> : stops.map((stop, index) => <div key={stop.id} className="rounded-2xl bg-slate-50 p-3"><div className="flex items-center gap-3"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white text-xs font-bold shadow-sm">{index + 1}</div><div className="min-w-0 flex-1"><div className="font-semibold">{stop.name}</div><div className="text-xs text-slate-500">{stop.province} · {stop.latitude.toFixed(3)}, {stop.longitude.toFixed(3)}</div></div><button onClick={() => move(index, -1)} disabled={index === 0} title="Move up" className="rounded-lg p-2 text-slate-500 disabled:opacity-30"><ArrowUp size={16}/></button><button onClick={() => move(index, 1)} disabled={index === stops.length - 1} title="Move down" className="rounded-lg p-2 text-slate-500 disabled:opacity-30"><ArrowDown size={16}/></button><button onClick={() => markVisited(stop.id)} title="Mark visited" className="rounded-lg p-2 text-slate-500 hover:bg-white"><CheckCircle2 size={16}/></button><button onClick={() => remove(stop.id)} title="Remove" className="rounded-lg p-2 text-slate-500 hover:bg-white"><Trash2 size={16}/></button></div></div>)}</div><div className="mt-5 rounded-2xl border border-emerald-100 bg-emerald-50 p-4"><div className="text-sm font-bold">Digital tourism passport</div><p className="mt-1 text-xs leading-5 text-slate-600">Self-reported visits remain unverified unless a governed verification mechanism confirms them.</p><div className="mt-3 space-y-2">{Object.entries(visited).length === 0 ? <div className="text-xs text-slate-500">No visits recorded yet.</div> : Object.entries(visited).map(([id, state]) => <div key={id} className="flex items-center gap-2 text-xs font-semibold"><CheckCircle2 size={14}/> {places.find(place => place.id === id)?.name ?? 'Published destination'} · visited{state.verified ? ' · verified' : ' · not verified'}</div>)}</div></div></section>
    </div>
