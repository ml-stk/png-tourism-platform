...
import { ArrowDown, ArrowUp, CheckCircle2, Compass, Link2, MapPin, Navigation, Plus, RotateCcw, Search, Trash2, WifiOff } from 'lucide-react';
...
const places: Place[] = [
  ...
  { id: 'tufi', name: 'Tufi', province: 'Oro', description: 'Fjords, reefs, diving and community-led coastal experiences.', latitude: -9.075, longitude: 149.323 },
];

const itineraryKey = 'png-tourism:visitor-itinerary:v2';
const passportKey = 'png-tourism:visitor-passport:v1';
...
export default function TripPlanner() {
  ...
  const provinces = useMemo(() => ['All', ...Array.from(new Set(places.map(place => place.province)))], []);
  const filtered = useMemo(() => places.filter(place =>
    !stops.some(stop => stop.id === place.id) &&
    (province === 'All' || place.province === province) &&
    `${place.name} ${place.description}`.toLowerCase().includes(query.toLowerCase().trim())
  ), [province, query, stops]);
  ...
  const createHandoff = (id: string) => {
    const uri = `pngtourism://destination/${id}`;
    setHandoff(uri);
    setNotice('QR/deep-link handoff prepared from published destination state.');
  };
  ...
  return <div className="space-y-6">
    <section className="rounded-3xl bg-slate-900 p-7 text-white lg:p-9">
      <div className="flex flex-col gap-5 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className={`inline-flex w-fit items-center gap-2 rounded-full px-3 py-2 text-xs font-semibold ${offline ? 'bg-amber-100 text-amber-900' : 'bg-emerald-100 text-emerald-900'}`}>{offline ? <WifiOff size={14}/> : <CheckCircle2 size={14}/>} {offline ? 'Offline — local state only' : 'Connected — local state saved'}</div>
      </div>
    </section>
    ...
    <div className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 lg:p-6">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h3 className="text-xl font-bold">Discover destinations</h3><p className="mt-1 text-sm text-slate-500">Published visitor content only.</p></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold">{filtered.length} available</span></div>
