...
export default function TripPlanner() {
  ...
  useEffect(() => { void loadPublishedData(); const on = () => { setOffline(false); void loadPublishedData(); }; const off = () => { setOffline(true); setDataStatus('fallback'); }; window.addEventListener('online', on); window.addEventListener('offline', off); return () => { window.removeEventListener('online', on); window.removeEventListener('offline', off); }; }, []);
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
      <section className="rounded-2xl border border-slate-200 bg-white p-5 lg:p-6"><div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between"><div><h3 className="text-xl font-bold">Discover destinations</h3><p className="mt-1 text-sm text-slate-500">Live published visitor projection, with governed offline fallback.</p></div><span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold">{filtered.length} available</span></div><div className="mt-5 grid gap-3 sm:grid-cols-[1fr_auto]"><label className="flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2"><Search size={16} className="text-slate-400"/><input aria-label="Search destinations" value={query} onChange={event => setQuery(event.target.value)} placeholder="Search destinations" className="w-full bg-transparent text-sm outline-none"/></label><select aria-label="Filter province" value={province} onChange={event => setProvince(event.target.value)} className="rounded-xl border border-slate-200 px-3 py-2 text-sm">{provinces.map(item => <option key={item}>{item}</option>)}</select></div><div className="mt-5 grid gap-4 md:grid-cols-2">{filtered.map(place => <article key={place.id} className="rounded-2xl border border-slate-200 p-5"><div className="flex items-start justify-between gap-3"><div><h4 className="font-bold">{place.name}</h4><div className="mt-1 text-xs font-semibold uppercase tracking-wide text-slate-500">{place.province}</div></div><MapPin size={18} className="text-emerald-700"/></div><p className="mt-3 text-sm leading-6 text-slate-600">{place.description}</p><div className="mt-4 flex flex-wrap gap-2"><button onClick={() => add(place)} className="inline-flex items-center gap-2 rounded-xl bg-slate-900 px-3 py-2 text-sm font-semibold text-white"><Plus size={15}/> Add to trip</button><button onClick={() => createHandoff(place.id)} className="inline-flex items-center gap-2 rounded-xl border border-slate-200 px-3 py-2 text-sm font-semibold"><Link2 size={15}/> QR handoff</button></div></div></article>)}</div>{loading && <div className="mt-4 text-center text-xs text-slate-400">Refreshing published destination data…</div>}{filtered.length === 0 && !loading && <div className="mt-5 rounded-2xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">No published destinations match your search.</div>}</section>
      <section className="rounded-2xl border border-slate-200 bg-white p-5 lg:p-6"><div className="flex items-center justify-between"><div><h3 className="text-xl font-bold">Your journey</h3><p className="mt-1 text-sm text-slate-500">{stops.length} destination stop{stops.length === 1 ? '' : 's'} · {plannedExperienceRecords.length} experience{plannedExperienceRecords.length === 1 ? '' : 's'} · offline-capable</p></div>{(stops.length > 0 || plannedExperiences.length > 0) && <button onClick={reset} className="inline-flex items-center gap-1 text-xs font-semibold text-slate-500"><RotateCcw size={13}/> Reset</button>}</div><div className="mt-5 space-y-3">{stops.length === 0 && plannedExperienceRecords.length === 0 ? <div className="rounded-2xl border border-dashed border-slate-300 p-8 text-center text-sm text-slate-500">Add a destination or published experience to start your itinerary.</div> : <>{stops.map((stop, index) => <div key={stop.id} className="rounded-2xl bg-slate-50 p-3"><div className="flex items-center gap-3"><div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-white text-xs font-bold shadow-sm">{index + 1}</div><div className="min-w-0 flex-1"><div className="font-semibold">{stop.name}</div><div className="text-xs text-slate-500">{stop.province} · {stop.latitude.toFixed(3)}, {stop.longitude.toFixed(3)}</div></div><button onClick={() => move(index, -1)} disabled={index === 0} title="Move up" className="rounded-lg p-2 text-slate-500 disabled:opacity-30"><ArrowUp size={16}/></button><button onClick={() => move(index, 1)} disabled={index === stops.length - 1} title="Move down" className="rounded-lg p-2 text-slate-500 disabled:opacity-30"><ArrowDown size={16}/></button><button onClick={() => markVisited(stop.id)} title="Mark visited" className="rounded-lg p-2 text-slate-500 hover:bg-white"><CheckCircle2 size={16}/></button><button onClick={() => remove(stop.id)} title="Remove" className="rounded-lg p-2 text-slate-500 hover:bg-white"><Trash2 size={16}/></button></div></div>)}{plannedExperienceRecords.map(experience => <div key={experience.id} className="rounded-2xl border border-emerald-100 bg-emerald-50 p-3"><div className="flex items-center gap-3"><div className="grid h-8 w-8 shrink-0 place-items-center rounded-full bg-white text-xs font-bold">E</div><div className="min-w-0 flex-1"><div className="font-semibold">{experience.title}</div><div className="text-xs text-slate-500">Published experience · {experience.provinceCode}</div></div><button onClick={() => removeExperience(experience.id)} title="Remove experience" className="rounded-lg p-2 text-slate-500 hover:bg-white"><Trash2 size={16}/></button></div></div>)}</>}</div><div className="mt-5 rounded-2xl border border-emerald-100 bg-emerald-50 p-4"><div className="text-sm font-bold">Digital tourism passport</div><p className="mt-1 text-xs leading-5 text-slate-600">Self-reported visits remain unverified unless a governed verification mechanism confirms them.</p><div className="mt-3 space-y-2">{Object.entries(visited).length === 0 ? <div className="text-xs text-slate-500">No visits recorded yet.</div> : Object.entries(visited).map(([id, state]) => <div key={id} className="flex items-center gap-2 text-xs font-semibold"><CheckCircle2 size={14}/> {places.find(place => place.id === id)?.name ?? 'Published destination'} · visited{state.verified ? ' · verified' : ' · not verified'}</div>)}</div></div></section>
    </div>
