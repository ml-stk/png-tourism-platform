...
import { destinationMediaFallbacks } from './destinationMediaFallbacks';
...
function governedFallback(): Destination[] {
  return fallback.filter(d => d.status === 'Published').map(d => {
    const slug = d.name.toLowerCase().replace(/[^a-z0-9]+/g, '-');
    return withFallbackMedia({ id: d.id, slug, name: d.name, provinceCode: d.province, description: d.description, contentVersion: 1, updatedAt: new Date().toISOString(), freshness: 'fresh', media: [], qrPath: `/destination/${d.name}`, offlineCacheKey: `destination:${d.id}:v1` });
  });
}
