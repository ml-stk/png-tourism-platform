  ...
  return <div className="space-y-5">
    ...
    <div className="grid gap-5 lg:grid-cols-[1.5fr_0.75fr]">
      <section className="overflow-hidden rounded-3xl border border-slate-200 bg-white">
        ...
        <div className="border-t border-slate-100 p-4"><div className="flex flex-wrap gap-2 pb-3">{suggestions.map(suggestion => <button key={suggestion} onClick={() => ask(suggestion)} disabled={busy || offline} className="rounded-full border border-slate-200 px-3 py-2 text-xs font-semibold text-slate-600 hover:bg-slate-50 disabled:cursor-not-allowed disabled:opacity-50">{suggestion}</button>)}</div><form onSubmit={e => {e.preventDefault(); ask();}} className="flex items-center gap-2 rounded-2xl border border-slate-200 bg-slate-50 p-2"><input value={message} onChange={e => setMessage(e.target.value)} placeholder="Ask the PNG Tourism Concierge…" disabled={busy || offline} className="min-w-0 flex-1 bg-transparent px-3 py-2 text-sm outline-none placeholder:text-slate-400"/><button type="submit" disabled={!message.trim() || busy || offline} className="rounded-xl bg-slate-950 p-3 text-white disabled:cursor-not-allowed disabled:opacity-40"><Send size={17}/></button></form></div>
      </section>
