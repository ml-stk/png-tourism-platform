...
const DEEP_LINK_PREFIX = '#industry-experience/';
...
export default function IndustryDiscovery() {
  ...
  const load = async () => {
    ...
    try {
      ...
      const publishedProfiles = nextProfiles.filter(item => item.published);
      const publishedExperiences = nextExperiences.filter(item => item.status === 'published');
      ...
      try {
        ...
        return value.experiences.filter(item => item.status === 'published');
      } catch { setError('Industry discovery is unavailable right now.'); return []; }
    }
  };
  ...
  useEffect(() => {
    void (async () => {
      ...
      if (hash.startsWith(DEEP_LINK_PREFIX)) {
        const experienceId = decodeURIComponent(hash.slice(DEEP_LINK_PREFIX.length));
        const target = loaded.find(item => item.id === experienceId);
        if (target) setSelected(target);
        else if (loaded.length) setError('The linked experience is not currently published.');
      }
    })();
  ...
  const selectExperience = (item: Experience) => {
    setSelected(item);
    setSubmitted(false);
    setMessage('');
    setError('');
    window.history.replaceState(null, '', `${window.location.pathname}${window.location.search}#${DEEP_LINK_PREFIX.slice(1)}${encodeURIComponent(item.id)}`);
  };
  const closeExperience = () => {
    setSelected(null); setSubmitted(false); setError('');
  ...
  if (selected) return <div className="space-y-6">
    <button onClick={closeExperience} className="inline-flex items-center gap-2 text-sm font-semibold text-slate-600"><ArrowLeft size={16}/> Back to industry discovery</button>
