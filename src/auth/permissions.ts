...
const ROLE_PERMISSIONS: Record<RoleCode, readonly PermissionCode[]> = {
  platform_admin: ['operator:read','operator:register','operator:approve','operator:manage_profile','operator:manage_compliance','operator:manage_status','content:read','content:write','content:publish','intelligence:read','admin:manage_users','audit:read',...ENTERPRISE],
  tpa_regulator: ['operator:read','operator:register','operator:approve','operator:manage_profile','operator:manage_compliance','operator:manage_status','audit:read','membership:read','membership:write','commerce:read','commerce:write','warehouse:read','warehouse:write','gis:read','gis:write','distribution:read','distribution:write'],
